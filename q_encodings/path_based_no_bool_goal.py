# Irfansha Shaik, 25.04.2022, Aarhus.
...
class PathBasedNoBoolGoal:
  ...
  def generate_goal_gate(self):
    ...
    for i in range(self.safe_max_path_length - 1):
      ...
      # Either the next position is same as current position (stuttering):
      self.encoding.append(['# Equality clause for the current path variables and next path variables: '])
      self.gates_generator.complete_equality_gate(self.goal_path_variables[i], self.goal_path_variables[i+1])
      next_cur_path_equality_output_gate = self.gates_generator.output_gate

      # Or Equality for neighbour variables and next position:
      ...
      # Disjunction for either of the above constraints:
      self.encoding.append(['# Disjunction between both the above constraints: '])
      self.gates_generator.or_gate([next_cur_path_equality_output_gate, path_neighbour_equality_output_gate])
      disjunction_equality_output_gate = self.gates_generator.output_gate

      # the position must be occupied and the color must be black:
      self.encoding.append(['# The goal position is occupied and the color is black and disjunction constraints are true: '])
      self.gates_generator.and_gate([self.predicate_variables[-1][0], -self.predicate_variables[-1][1], disjunction_equality_output_gate])
    ...
    self.encoding.append(['# if path starts stuttering, it keeps on stuttering : '])
