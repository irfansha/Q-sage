...
# Open the games list:
f = open('all_games.txt','r')
lines = f.readlines()
f.close()

# First player winning file:
f_first_player = open('19_neighbours_first_player.txt','r')
first_player_lines = f_first_player.readlines()
f_first_player.close()

# Second player winning file:
f_second_player = open('19_neighbours_second_player.txt','r')
...
for line in lines:
  ...
  # Separating first and second player moves, while converting the moves to the board idicies:
  ...
  if (swap_flag == 'swapped'):
    f = open("B-Hex/" + str(game_id) + "_move_" + str(int(game_length) - unrolling_depth + 1) + '.pg', 'w')
  else:
    f = open("B-Hex/" + str(game_id) + "_move_" + str(int(game_length) - unrolling_depth) + '.pg', 'w')  
